...
const signup = async (req, res, next) => {
    try {
        ...
        // Définir le rôle par défaut sur "user"
        const defaultRole = "user";

        // Logique pour attribuer le rôle d'administrateur si nécessaire
...
const updateUserRole = async (req, res, next) => {
    try {
        ...
        if (updateSuccess) {
            ...
            return res.status(403).json({ message: "Le rôle 'admin' ne peut pas être mis à jour." });
        }
    } catch (error) {
        console.error(error);
        return res.status(500).json({
